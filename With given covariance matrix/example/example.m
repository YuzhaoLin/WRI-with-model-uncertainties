...
for f = f0:df:nf
    ...
    tmp = ['D_' num2str(ki) ];
    dobs = eval(tmp);
    fh = @(m)misfit(m,dobs,alpha,model,sigmp,sigmm);
 
    % Simple BB iteration
    [mk,hist] = BBiter(fh,mk,1e-6,5);     
...
figure;for f = f0:df:nf
           ...
           plot(hist(:,2),'LineWidth',2);axis([0,length(hist(:,2)),0,ymax]);
...
print(3,'-depsc','-r300',['../Fig/v_fwi_un']);
print(4,'-depsc','-r300',['../Fig/vel_60']);
